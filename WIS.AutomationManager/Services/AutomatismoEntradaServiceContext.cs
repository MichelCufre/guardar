...
{
    public class AutomatismoEntradaServiceContext : AutomatismoServiceContext
    {
        protected int _userId = 0;

        protected readonly EntradaStockAutomatismoRequest _request;

        public readonly Dictionary<string, Producto> Productos = new Dictionary<string, Producto>();

        public AutomatismoEntradaServiceContext(IUnitOfWork uow,
            EntradaStockAutomatismoRequest request,
            int userId) : base(uow)
        {
            _request = request;
            _userId = userId;
