...
{
    public class ImpresionHerramientasValidationModule : FormValidationModule
	{
		...
		public ImpresionHerramientasValidationModule(IUnitOfWork uow, int usuarioLogueado, string predioLogueado, List<Ubicacion> ubicaciones = null)
		{
			this._uow = uow;
		...
		public virtual void ValidateImpresora_OnSucess(FormField field, Form form, List<ComponentParameter> parameters)
		{
			...
			if (!parameters.Any(s => s.Id == "isSubmit"))
			{
				...
				List<EtiquetaEstiloLenguaje> listaEstilos = _uow.ImpresionRepository.GetEstiloByLenguaje(lenguaje.Id, EstiloEtiquetaDb.Equipo);
