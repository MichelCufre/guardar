...
{
    public class DOC085FormValidationModule : FormValidationModule
    {
        ...
        public DOC085FormValidationModule(
            bool editMode,
            IUnitOfWork uow,
        ...
        {
            var tpDoc = form.GetField("tpIngreso").Value;

            if (!this._editMode && !this._uow.DocumentoTipoRepository.DocumentoNumeracionAutogenerada(tpDoc))
            {

                if (string.IsNullOrEmpty(field.Value) && parameters.Any(a => a.Id == "nroDocNoGenerado"))
                {
                    //tratamiento de mascara
                    string value = parameters.FirstOrDefault(a => a.Id == "nroDocNoGenerado").Value;

                    if (!string.IsNullOrEmpty(value) && value.Length == 12)
                    {
                        value = value.Remove(10, 1);
                ...
                return new FormValidationGroup
                {
                    ...
                    Rules = new List<IValidationRule>
                    {
                        ...
                        new PositiveNumberMaxLengthValidationRule(field.Value, 99),
                        new NumeroDocumentoIngresoNoAutogeneradoValidationRule(tpDoc, field.Value, this._uow)
                    }
                };
            }
            else
