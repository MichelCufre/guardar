...
{
    public class CreateAtributoTipoLpnDetFormValidationModule : FormValidationModule
    {
        ...
        public CreateAtributoTipoLpnDetFormValidationModule(IUnitOfWork uow, IIdentityService identity, ISecurityService security, IFormatProvider provider)
        {
            this._uow = uow;
        ...
        public virtual void AtributoOnSuccess(FormField field, Form form, List<ComponentParameter> parameters)
        {
            ...
            switch (atributo.IdTipo)
            {
                ...
                case TipoAtributoDb.DOMINIO:
                    ...
                    foreach (var det in detallesDominio)
                    {
                        selectConsolidadorTipo.Options.Add(new SelectOption(det.Id.ToString(), $"{det.Id} - {det.Descripcion}"));
                    }
