...
{
    public class FormulaSalidaGridValidationModule : GridValidationModule
    {
        protected readonly bool _forceNullValidationOnNonModified;
        protected readonly IUnitOfWork _uow;
        protected readonly IFormatProvider _culture;

        public FormulaSalidaGridValidationModule(IUnitOfWork uow, IFormatProvider culture, bool forceNullValidationOnNonModified = false)
        {
            this._uow = uow;
        ...
        {
            string empresaParam = parameters.Where(d => d.Id == "empresa").FirstOrDefault().Value;

            int empresa = int.Parse(empresaParam);
        ...
        public virtual void Validate(List<GridRow> rows, List<ComponentParameter> parametros)
        {
            ...
            var codigoFormula = parametros.Any(s => s.Id == "codigo") ? parametros.FirstOrDefault(s => s.Id == "codigo").Value : string.Empty;
            ...
            if (formula == null && rows.Count == 0)
                throw new ValidationFailedException("PRD100_grid1_error_SalidaVacia");
            else if (!string.IsNullOrEmpty(codigoFormula) && formula.Salida.Count == 0 && rows.Count == 0)
                throw new ValidationFailedException("PRD100_grid1_error_SalidaVacia");
