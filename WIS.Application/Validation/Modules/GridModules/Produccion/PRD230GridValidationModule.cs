...
{
    public class PRD230GridValidationModule : GridValidationModule
    {
        protected readonly IUnitOfWork _uow;
        protected readonly IFormatProvider _culture;

        public PRD230GridValidationModule(IUnitOfWork uow,
            ...
            if (producto.AceptaDecimales)
            {
                return new GridValidationGroup
                {
                    ...
                    Rules = new List<IValidationRule>
                    {
                        ...
                        new SaldoStockSuficienteEntradaBBValidationRule(_uow,
                            row.GetCell("QT_MOVIMIENTO_BB").Value,
                            row.GetCell("QT_RECHAZO_AVERIA").Value,
                            "0",
                            PRD230.ObtenerTipoStockSalida(parameters) == TipoStockOutBB.INSUMO ? row.GetCell("QT_RESERVA_SAIDA").Value : row.GetCell("QT_ESTOQUE").Value,_culture)
                    }
                };
            }
