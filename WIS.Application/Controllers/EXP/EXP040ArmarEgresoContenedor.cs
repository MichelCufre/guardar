...
{
    public class EXP040ArmarEgresoContenedor : AppController
    {
        ...
        public EXP040ArmarEgresoContenedor(
            ...
            ITrackingService trackingService)
        {
            ...
            {
                "CD_CLIENTE", "CD_EMPRESA", "NU_PREPARACION", "NU_CONTENEDOR", "NU_CARGA", "CD_GRUPO_EXPEDICION"
            };
        ...
        {
            if (this._security.IsUserAllowed(SecurityResources.WEXP040_grid1_btn_ArmarCamionCont))
            {
                if (grid.Id == "AgregarContenedor_grid_1")
                    grid.MenuItems.Add(new GridButton("btnAgregar", "General_Sec0_btn_Agregar"));
                else if (grid.Id == "QuitarContenedor_grid_2")
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            if (grid.Id == "AgregarContenedor_grid_1")
            {
                var dbQuery = new ArmarCamionContenedorQuery(camion, empresa, ruta);
                uow.HandleQuery(dbQuery);
                grid.Rows = _gridService.GetRows(dbQuery, grid.Columns, context, this.DefaultSort, this.GridKeys);
            }
            else if (grid.Id == "QuitarContenedor_grid_2")
            {
                var dbQuery = new ArmarCamionContenedorQuitarQuery(camion, empresa, ruta);
                uow.HandleQuery(dbQuery);
                grid.Rows = _gridService.GetRows(dbQuery, grid.Columns, context, this.DefaultSort, this.GridKeys);
            }
        ...
        {
            if (context.GridId == "AgregarContenedor_grid_1" && context.ButtonId == "btnAgregar")
                this.ProcesarAgregar(context);
            else if (context.GridId == "QuitarContenedor_grid_2" && context.ButtonId == "btnQuitar")
        ...
        public virtual void ProcesarAgregar(GridMenuItemActionContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            uow.CreateTransactionNumber("ArmarEgresoContenedor: Agregar");
            ...
            try
            {
                ...
                var contenedores = selection.Select(item => new ContenedorAsociarUnidad
                {
                    Carga = long.Parse(item["NU_CARGA"]),
                    Empresa = int.Parse(item["CD_EMPRESA"]),
                    Preparacion = int.Parse(item["NU_PREPARACION"]),
                    Contenedor = int.Parse(item["NU_CONTENEDOR"]),
                ...
                {
                    var dbQuery = new ArmarCamionContenedorQuery(camion.Id, camion.Empresa, camion.Ruta);
                    uow.HandleQuery(dbQuery);
                    dbQuery.ApplyFilter(this._filterInterpreter, context.Filters);

                    var contenedoresSeleccion = dbQuery.GetContenedoresUnidad();
                    contenedores = contenedoresSeleccion.Except(contenedores).ToList();
                }

                var armadoEgreso = new ArmadoEgresoContenedor(uow, _factoryService, _identity, camion, expedicionService, contenedores);
                armadoEgreso.Armar();
        ...
        public virtual void ProcesarQuitar(GridMenuItemActionContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            uow.CreateTransactionNumber("ArmarEgresoContenedor: Quitar");
            uow.BeginTransaction();
            ...
            try
            {
                ...
                if (context.Selection.AllSelected)
                {
                    var dbQuery = new ArmarCamionContenedorQuitarQuery(camion.Id, camion.Empresa, camion.Ruta);
                    uow.HandleQuery(dbQuery);
                    dbQuery.ApplyFilter(this._filterInterpreter, context.Filters);
                ...
                var desarmadoEgreso = new DesarmadoEgresoContenedor(uow, _factoryService, _identity, camion, expedicionService, contenedores);
