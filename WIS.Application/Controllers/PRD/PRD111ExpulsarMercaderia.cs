...
{
    public class PRD111ExpulsarMercaderia : AppController
    {
        ...
        public PRD111ExpulsarMercaderia(
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            var idEspacioProduccion = context.Parameters.FirstOrDefault(x => x.Id == "idEspacioProduccion").Value;

            var ubicacionProduccion = uow.ProduccionRepository.GetUbicacionProduccion(idEspacioProduccion);

            var dbQuery = new StockEspacioProduccionQuery(ubicacionProduccion);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext query)
        {
            ...
            var idEspacioProduccion = query.Parameters.FirstOrDefault(x => x.Id == "idEspacioProduccion").Value;

            var logicaProduccion = this._logicaProduccionFactory.GetLogicaProduccion(uow, idEspacioProduccion);
        ...
        public override GridMenuItemActionContext GridMenuItemAction(GridMenuItemActionContext context)
        {
            ...
            context.AddParameter("PRD111_ProductosExpulsable", JsonConvert.SerializeObject(stock));
        ...
        public override Grid GridCommit(Grid grid, GridFetchContext context)
        {
            ...
            {
                context.AddErrorNotification("PRD111_Sec0_Error_02");
                return grid;
            }
            var espacioProduccion = uow.EspacioProduccionRepository.GetEspacioProduccion(idEspacioProduccion);
            var metadata = context.Parameters.FirstOrDefault(x => x.Id == "idEspacioProduccion")?.Value;
            ...
            {
                context.AddErrorNotification("PRD111_Sec0_Error_03");
                return grid;
            }
            ...
            try
            {
                ...
                {
                    ImprimirEtiquetaTransferencia(uow, espacioProduccion.Predio, codigoImpresora, out impresion, out nuEtiquetaExterna);
                }

                uow.ProduccionRepository.ExpulsarProductos(uow, espacioProduccion.Predio, stocksExpulsar, expulsarConTransferencia, nuTransaccion, nuEtiquetaExterna, _identity.UserId);

                uow.SaveChanges();
                uow.Commit();

                if (expulsarConTransferencia)
                    context.AddSuccessNotification("PRD111_msg_Success_StockExpulsadoEtiqueta", new List<string> { nuEtiquetaExterna.ToString() });
                else
        ...
        {
            using var uow = this._uowFactory.GetUnitOfWork();
            var codigoEspacio = context.GetParameter("idEspacioProduccion");
            ...
            this.InicializarSelect(uow, form, context.Parameters);
        ...
        public virtual void ImprimirEtiquetaTransferencia(IUnitOfWork uow, string predio, string codImpresora, out Impresion impresion, out decimal? nuEtiquetaExterna)
        {
            var cantidadGenerar = 1;
            var transferencias = new List<EtiquetaTransferencia> { new EtiquetaTransferencia() };

            var impresora = uow.ImpresoraRepository.GetImpresora(codImpresora, predio);
            ...
            ImpresionBuilder builder = new ImpresionBuilder(uow.ImpresoraRepository.GetImpresora(codImpresora, predio), strategy, _printingService);
        ...
        public virtual void AgregarBloqueos(List<ProductosExpulsable> stocksExpulsar, TrafficOfficerTransaction transactionTO)
        {
            ...
            if (listLock.Count > 0)
            {
                ...
                throw new EntityLockedException("PRD111_msg_Error_StockBloqueada", new string[] { keyBloqueo[2], keyBloqueo[4] });
            }
        ...
        {
            int rowId = 0;
            List<ProductosExpulsable> stocksSeleccionado = JsonConvert.DeserializeObject<List<ProductosExpulsable>>(context.Parameters.FirstOrDefault(x => x.Id == "PRD111_ProductosExpulsable").Value);
        ...
        public virtual void InicializarSelect(IUnitOfWork uow, Form form, List<ComponentParameter> parameters)
        {
            ...
            var idEspacioP = uow.EspacioProduccionRepository.GetEspacioProduccion(parameters.FirstOrDefault(x => x.Id == "idEspacioProduccion").Value);

            listaImpresoras = uow.ImpresoraRepository.GetListaImpresorasPredio(idEspacioP.Predio);
