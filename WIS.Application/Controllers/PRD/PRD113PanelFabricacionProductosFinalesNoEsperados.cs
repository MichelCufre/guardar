...
{
	public class PRD113PanelFabricacionProductosFinalesNoEsperados : AppController
	{
		...
		public PRD113PanelFabricacionProductosFinalesNoEsperados(IIdentityService identity, IGridValidationService gridValidationService, IUnitOfWorkFactory uowFactory, IGridService gridService, IGridExcelService excelService, IFormValidationService formValidationService, IFilterInterpreter filterInterpreter)
		{
		...
		{
			this.GridKeys = new List<string> { "NU_PRDC_SALIDA_REAL" };
			...
			var dbQuery = new ProductosFinalesNoEsperadosProduccionQuery(nroIngresoProduccion, empresa);

			uow.HandleQuery(dbQuery);

			var defaultSort = new List<SortCommand> { new SortCommand("NU_PRDC_SALIDA_REAL", SortDirection.Ascending) };
		...
		public override Grid GridCommit(Grid grid, GridFetchContext query)
		{
			...
			{
				query.AddErrorNotification(ex.Message, ex.StrArguments == null ? new List<string>() : ex.StrArguments.ToList());
			}
			catch (Exception ex)
			{
