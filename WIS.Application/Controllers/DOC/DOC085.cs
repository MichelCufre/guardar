...
{
    public class DOC085 : AppController
    {
        ...
        public DOC085(ISessionAccessor session,
        ...
        public override Form FormSubmit(Form form, FormSubmitContext context)
        {
                    ...
                    if (documento != null)
                    {
                        using (var uow = this._uowFactory.GetUnitOfWork())
                        {
                            if (documento.CanEdit(uow))
                            {
                                ...
                                context.AddSuccessNotification("DOC085_Sec0_Error_Error01");
                            }
                            else
                            {
                                context.AddErrorNotification("DOC085_Sec0_Error_DocumentoNoEditable");
                            }
                        }
                    }
                    else
                    {
                        context.AddErrorNotification("DOC085_Sec0_Error_Error02");
                    }
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                context.AddErrorNotification("DOC085_Sec0_Error_Error03", new List<string> { ex.Message });
            }
            finally
            {
                context.Redirect("/documento/DOC085", new List<ComponentParameter>() { });
        ...
        public override Form FormValidateForm(Form form, FormValidationContext context)
        {
            ...
            return this._formValidationService.Validate(new DOC085FormValidationModule(editMode, uow, this._identity), form, context);
        }
        ...
        public virtual void InicializarSelects(Form form)
        {
            ...
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                var documentosIngreso = uow.DocumentoTipoRepository.GetDocumentosIngresoManualHabilitados();
            ...
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                ...
                {
                    selectTransportadora.Options.Add(new SelectOption(transportista.Id.ToString(), transportista.Descripcion));
                }
        ...
        public virtual void InicializarModoEdit(Form form, FormInitializeContext context)
        {
            ...
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                ...
                if (doc != null)
                {
                    ...
                    form.GetField("cdVia").Value = doc.Via;
                    ...
                    if (empresa != null)
                    {
                        ...
                        if (doc.Cliente != null)
                        {
                            Agente cliente = uow.AgenteRepository.GetAgente(empresa.Id, AgenteTipo.Proveedor, doc.Cliente);

                            if (cliente != null)
                            {
                                fieldProovedor.Options.Add(new SelectOption(cliente.Codigo, cliente.Descripcion));
                                fieldProovedor.Value = cliente.Codigo;
                            }
        ...
        public virtual List<SelectOption> SearchEmpresa(Form form, FormSelectSearchContext context)
        {
            ...
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                ...
                {
                    opciones.Add(new SelectOption(empresa.Id.ToString(), empresa.Nombre));
                }
            }
        ...
        public virtual List<SelectOption> SearchCliente(Form form, FormSelectSearchContext context)
        {
            ...
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                if (int.TryParse(form.GetField("cdEmpresa").Value, out int cdEmpresa))
                {
                    ...
                    {
                        opciones.Add(new SelectOption(agente.Codigo, agente.Descripcion));
                    }
                }
            }
        ...
        public virtual List<SelectOption> SearchDespachante(Form form, FormSelectSearchContext context)
        {
            ...
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                ...
                {
                    opciones.Add(new SelectOption(despachante.Id.ToString(), despachante.Nombre));
                }
            }
        ...
        public virtual List<SelectOption> SearchUnidadMedida(Form form, FormSelectSearchContext context)
        {
            ...
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                var unidadesMedida = uow.UnidadMedidaRepository.GetByNombreOrCodePartial(context.SearchValue);
        ...
        public virtual List<SelectOption> SearchAlmacenajeSeguro(Form form, FormSelectSearchContext context)
        {
            List<SelectOption> opciones = new List<SelectOption>();

            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                var tipos = uow.TipoAlmacenajeSeguroRepository.GetTipoAlmacenajeSeguroByNamePartial(context.SearchValue);

                foreach (var almacenajeSeguro in tipos)
                {
                    opciones.Add(new SelectOption(almacenajeSeguro.Tipo.ToString(), almacenajeSeguro.Descripcion));
                }
            }

            return opciones;
        }
        public virtual List<SelectOption> SearchVia(Form form, FormSelectSearchContext context)
        {
            List<SelectOption> opciones = new List<SelectOption>();

            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                List<Via> vias = uow.ViaRepository.GetViaByNombrePartial(context.SearchValue);

                foreach (var via in vias)
                {
                    opciones.Add(new SelectOption(via.Id, via.Descripcion));
                }
            }
        ...
        public virtual List<SelectOption> SearchTransportadora(Form form, FormSelectSearchContext context)
        {
            ...
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                var transportistas = uow.TransportistaRepository.GetByDescripcionOrCodePartial(context.SearchValue);
        ...
        public virtual List<SelectOption> SearchMoneda(Form form, FormSelectSearchContext context)
        {
            List<SelectOption> opciones = new List<SelectOption>();

            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                List<Moneda> monedas = uow.MonedaRepository.GetMonedaByNombre(context.SearchValue);

                foreach (var moneda in monedas)
                {
                    opciones.Add(new SelectOption(moneda.Codigo, moneda.Descripcion));
                }
            }
        ...
        public virtual void CrearDocumento(Form form, FormSubmitContext context)
        {
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                uow.CreateTransactionNumber("DOC085 CrearDocumento");
                ...
                context.AddSuccessNotification("DOC085_Sec0_Error_Error04", new List<string> { documento.Numero, documento.Tipo });
            }
        }
        ...
        public virtual IDocumentoIngreso CrearDocumentoObject(Form form, IUnitOfWork uow)
        {
            ...
            var documento = this._factoryService.CreateDocumentoIngreso(tipoDocumento.TipoDocumento);
            ...
            {
                documento.Numero = "";
                documento.Tipo = tipoDocumento.TipoDocumento;
                ...
                if (uow.DocumentoTipoRepository.DocumentoNumeracionAutogenerada(tipoDocumento.TipoDocumento))
                {
        ...
        public virtual IDocumentoIngreso EditarDocumentoObject(Form form)
        {
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                var tipoDocumento = uow.DocumentoTipoRepository.GetTipoDocumento(form.GetField("tpIngreso").Value);
                var documento = uow.DocumentoRepository.GetIngreso(form.GetField("nroDoc").Value, tipoDocumento.TipoDocumento);
                var culture = this._identity.GetFormatProvider();
                ...
                if (documento != null)
                {
                    ...
                    decimal arbitraje;
                    if (decimal.TryParse(form.GetField("vlArbitraje").Value, NumberStyles.Number, culture, out arbitraje))
                        documento.ValorArbitraje = arbitraje;
                    ...
                    if (short.TryParse(form.GetField("cdDespachante").Value, NumberStyles.Number, culture, out short cdDespachante))
                        documento.Despachante = cdDespachante;
                    ...
                    if (short.TryParse(form.GetField("qtContenedor20").Value, out qtContenedor20))
                        documento.CantidadContenedor20 = qtContenedor20;
        ...
        public override Grid GridInitialize(Grid grid, GridInitializeContext context)
        {
            grid.AddOrUpdateColumn(new GridColumnItemList("BTN_LIST", new List<IGridItem> {
                new GridItemHeader("DOC085_Sec0_lbl_Acciones"),
                new GridButton("btnDetalles", "General_Sec0_btn_Detalles", "fas fa-list"),
                new GridButton("btnSaldo", "DOC085_Sec0_btn_Saldos", "fas fa-clipboard-list"),
                new GridItemDivider(),
        ...
        public override GridButtonActionContext GridButtonAction(GridButtonActionContext context)
        {
            ...
            else if (context.ButtonId == "btnSaldo")
            {
                context.Redirect("/documento/DOC020", new List<ComponentParameter>()
                {
                    new ComponentParameter(){ Id = "nuDocumento", Value = context.Row.GetCell("NU_DOCUMENTO").Value },
                    new ComponentParameter(){ Id = "tpDocumento", Value = context.Row.GetCell("TP_DOCUMENTO").Value },
                    new ComponentParameter(){ Id = "cdEmpresa", Value = context.Row.GetCell("CD_EMPRESA").Value },
