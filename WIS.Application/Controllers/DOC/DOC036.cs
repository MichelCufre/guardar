...
{
    public class DOC036 : AppController
    {
        ...
        public DOC036(ISessionAccessor session,
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            {
                var query = new DetallesEgresoQuery();
        ...
        public override byte[] GridExportExcel(Grid grid, GridExportExcelContext context)
        {
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                var dbQuery = new DetallesEgresoQuery();

                uow.HandleQuery(dbQuery);

                var defaultSort = new SortCommand("DT_ADDROW", SortDirection.Descending);

                context.FileName = "DetallesEgreso_" + DateTime.Now.ToString("yyyy-MM-dd_HH:mm") + ".xlsx";
