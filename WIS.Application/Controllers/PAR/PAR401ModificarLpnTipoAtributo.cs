...
{
    public class PAR401ModificarLpnTipoAtributo : AppController
    {
        ...
        public PAR401ModificarLpnTipoAtributo(IIdentityService identity, IUnitOfWorkFactory uowFactory, IFormValidationService formValidationService, ISecurityService security)
        {
        ...
        public override Form FormInitialize(Form form, FormInitializeContext context)
        {
            using (var uow = this._uowFactory.GetUnitOfWork())
            {
                ...
                var lpnTipoAtributo = uow.ManejoLpnRepository.GetLpnTipoAtributo(int.Parse(idAtributo), lpnTipo);
                ...
                var listaConsolidadorTipo = uow.ManejoLpnRepository.GetListaLpnConsolidadorTipo();
                ...
                selectConsolidadorTipo.Value = lpnTipoAtributo.IdConsolidador.ToString();

                var tipoLpn = uow.ManejoLpnRepository.GetTipoLpn(lpnTipo);
                if (tipoLpn.PermiteConsolidar == "S")
                    context.Parameters.Add(new ComponentParameter { Id = "isRequiredConsolidado", Value = "T" });
                else
                    context.Parameters.Add(new ComponentParameter { Id = "isRequiredConsolidado", Value = "F" });
                ...
                switch (atributo.IdTipo)
                {
                    ...
                    case TipoAtributoDb.DOMINIO:
                        ...
                        foreach (var dominio in detallesDominio)
                        {
                ...
                form.GetField("ID_CONSOLIDACION_TIPO").Value = lpnTipoAtributo.IdConsolidador.ToString();
        ...
        public override Form FormSubmit(Form form, FormSubmitContext context)
        {
            ...
            try
            {
                ...
                var lpnTipo = context.Parameters.FirstOrDefault(x => x.Id == "LpnTipo");

                if (lpnTipo != null)
                    tplpn = lpnTipo.Value;
                
                var lpnTipoAtributo = uow.ManejoLpnRepository.GetLpnTipoAtributo(int.Parse(idAtributo), tplpn);

                lpnTipoAtributo.IdAtributo = int.Parse(idAtributo);
                lpnTipoAtributo.TipoLpn = tplpn;
                lpnTipoAtributo.Requerido = Mapper.MapStringToBooleanString(form.GetField("FL_REQUERIDO").Value);
                lpnTipoAtributo.ValidoInterfaz = Mapper.MapStringToBooleanString(form.GetField("VL_VALIDO_INTERFAZ").Value);
                lpnTipoAtributo.EstadoInicial = form.GetField("ID_ESTADO_INICIAL").Value;

                if (!string.IsNullOrEmpty(form.GetField("ID_CONSOLIDACION_TIPO").Value))
                    lpnTipoAtributo.IdConsolidador = int.Parse(form.GetField("ID_CONSOLIDACION_TIPO").Value);
                ...
                lpnTipoAtributo.ValorInicial = AtributoHelper.GetValorByIdTipo(form, atributo, separador, formaterDateTime);

                uow.ManejoLpnRepository.UpdateLpnTipoAtributo(lpnTipoAtributo);
