...
{
    public class REC170PlanificacionLpn : AppController
    {
        ...
        {
            if (grid.Id == "REC170PlanificacionLpn_grid_1")
                grid.MenuItems.Add(new GridButton("btnAgregar", "General_Sec0_btn_AgregarSeleccion"));
            else if (grid.Id == "REC170PlanificacionLpn_grid_2")
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            if (grid.Id == "REC170PlanificacionLpn_grid_1")
            {
                var dbQuery = new PlanificacionLpnQuery(agenda.IdEmpresa);
                uow.HandleQuery(dbQuery);
                grid.Rows = _gridService.GetRows(dbQuery, grid.Columns, context, this.DefaultSort, this.GridKeys);
            }
            else if (grid.Id == "REC170PlanificacionLpn_grid_2")
            {
                var dbQuery = new PlanificacionLpnQuery(agenda.IdEmpresa, agenda.Id);
                uow.HandleQuery(dbQuery);
                grid.Rows = _gridService.GetRows(dbQuery, grid.Columns, context, this.DefaultSort, this.GridKeys);
            }
        ...
        {
            if (context.GridId == "REC170PlanificacionLpn_grid_1" && context.ButtonId == "btnAgregar")
                ProcesarAgregar(context);
            else if (context.GridId == "REC170PlanificacionLpn_grid_2" && context.ButtonId == "btnQuitar")
                ProcesarQuitar(context);
        ...
        public virtual void ProcesarAgregar(GridMenuItemActionContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            uow.CreateTransactionNumber("REC170PlanificacionLpn: Agregar");

            var planificaciones = GetKeys(uow, context, nuAgenda);
            uow.AgendaRepository.GenerarPlanificacionAgendaLpn(nuAgenda, planificaciones, uow.GetTransactionNumber());
        }
        ...
        public virtual void ProcesarQuitar(GridMenuItemActionContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            uow.CreateTransactionNumber("REC170PlanificacionLpn: Quitar");

            var planificaciones = GetKeys(uow, context, nuAgenda, agregar: false);
            uow.AgendaRepository.GenerarPlanificacionAgendaLpn(nuAgenda, planificaciones, uow.GetTransactionNumber(), desasociar: true);
        }

        public virtual List<AgendaLpnPlanificacion> GetKeys(IUnitOfWork uow, GridMenuItemActionContext context, int nuAgenda, bool agregar = true)
        {
            ...
            var dbQuery = new PlanificacionLpnQuery(agenda.IdEmpresa, (agregar ? null : agenda.Id));
            uow.HandleQuery(dbQuery);
