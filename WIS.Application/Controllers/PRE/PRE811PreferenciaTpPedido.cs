...
{
    public class PRE811PreferenciaTpPedido : AppController
    {
        ...
        public PRE811PreferenciaTpPedido(
            ...
            IIdentityService identity)
        {
            ...
            {
                new SortCommand("TP_PEDIDO",SortDirection.Descending)
            };
        ...
        {
            if (grid.Id == "AgregarTpPedido_grid_1")
            {
                grid.MenuItems.Add(new GridButton("btnAgregar", "General_Sec0_btn_Agregar"));
            }
            else if (grid.Id == "QuitarTpPedido_grid_2")
            {
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            try
            {
                ...
                if (grid.Id == "AgregarTpPedido_grid_1")
                {
                    var dbQuery = new AgregarTpPedidoPreferenciaQuery(int.Parse(codigoPreferencia));
                ...
                else if (grid.Id == "QuitarTpPedido_grid_2")
                {
                    if (!string.IsNullOrEmpty(codigoPreferencia))
                    {
                        ...
                        var dbQuery = new QuitarTpPedidoPreferenciaQuery(preferencia.NU_PREFERENCIA);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            if (!string.IsNullOrEmpty(codigoPreferencia))
            {
                ...
                {
                    var dbQuery = new QuitarTpPedidoPreferenciaQuery(int.Parse(codigoPreferencia));
                    uow.HandleQuery(dbQuery);
                    dbQuery.ApplyFilter(this._filterInterpreter, context.Filters);
        ...
        public override byte[] GridExportExcel(Grid grid, GridExportExcelContext context)
        {
            ...
            if (grid.Id == "AgregarTpPedido_grid_1")
            {
                var dbQuery = new AgregarTpPedidoPreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
            ...
            {
                var dbQuery = new QuitarTpPedidoPreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
        ...
        {
            if (context.GridId == "AgregarTpPedido_grid_1" && context.ButtonId == "btnAgregar")
            {
                this.ProcesarAgregar(context);
            }
            else if (context.GridId == "QuitarTpPedido_grid_2" && context.ButtonId == "btnQuitar")
            {
        ...
        public virtual void ProcesarAgregar(GridMenuItemActionContext context)
        {
            ...
            try
            {
                ...
                List<PreferenciaAsociarTpPedido> tpPedido = new List<PreferenciaAsociarTpPedido>();
                ...
                if (context.Selection.AllSelected)
                    tpPedido = dbQuery.GetSelectedKeysAndExclude(context.Selection.Keys, nuPreferencia);
                else
                    tpPedido = dbQuery.GetSelectedKeys(context.Selection.Keys, nuPreferencia);

                uow.PreferenciaRepository.AddTpPedidoPreferencia(tpPedido);
                uow.SaveChanges();
                uow.Commit();
            }
            catch (ValidationFailedException ex)
        ...
        public virtual void ProcesarQuitar(GridMenuItemActionContext context)
        {
            ...
            try
            {
                ...
                uow.PreferenciaRepository.RemoveTpPedidoPreferencia(tpPedido);
                uow.SaveChanges();
                uow.Commit();
            }
            catch (ValidationFailedException ex)
