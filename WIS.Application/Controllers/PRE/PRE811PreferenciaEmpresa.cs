...
{
    public class PRE811PreferenciaEmpresa : AppController
    {
        ...
        public PRE811PreferenciaEmpresa(
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            try
            {
                ...
                if (grid.Id == "AgregarEmpresa_grid_1")
                {
                    var dbQuery = new AgregarEmpresaPreferenciaQuery(int.Parse(codigoPreferencia));
                ...
                else if (grid.Id == "QuitarEmpresa_grid_2")
                {
                    if (!string.IsNullOrEmpty(codigoPreferencia))
                    {
                        ...
                        var dbQuery = new QuitarEmpresaPreferenciaQuery(preferencia.NU_PREFERENCIA);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            if (!string.IsNullOrEmpty(codigoPreferencia))
            {
                ...
                {
                    var dbQuery = new QuitarEmpresaPreferenciaQuery(int.Parse(codigoPreferencia));
                    uow.HandleQuery(dbQuery);
                    dbQuery.ApplyFilter(this._filterInterpreter, context.Filters);
        ...
        public override byte[] GridExportExcel(Grid grid, GridExportExcelContext context)
        {
            ...
            if (grid.Id == "AgregarEmpresa_grid_1")
            {
                var dbQuery = new AgregarEmpresaPreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
            ...
            {
                var dbQuery = new QuitarEmpresaPreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
        ...
        {
            if (context.GridId == "AgregarEmpresa_grid_1" && context.ButtonId == "btnAgregar")
            {
                this.ProcesarAgregar(context);
            }
            else if (context.GridId == "QuitarEmpresa_grid_2" && context.ButtonId == "btnQuitar")
            {
        ...
        public virtual void ProcesarAgregar(GridMenuItemActionContext context)
        {
            ...
            try
            {
                ...
                List<PreferenciaAsociarEmpresa> empresa = new List<PreferenciaAsociarEmpresa>();
                ...
                if (context.Selection.AllSelected)
                    empresa = dbQuery.GetSelectedKeysAndExclude(context.Selection.Keys, nuPreferencia);
                else
                    empresa = dbQuery.GetSelectedKeys(context.Selection.Keys, nuPreferencia);

                uow.PreferenciaRepository.AddEmpresasPreferencia(empresa);
                uow.SaveChanges();
                uow.Commit();
            }
            catch (ValidationFailedException ex)
        ...
        public virtual void ProcesarQuitar(GridMenuItemActionContext context)
        {
            ...
            try
            {
                ...
                uow.PreferenciaRepository.RemoveEmpresasPreferencia(empresa);
                uow.SaveChanges();
                uow.Commit();
            }
            catch (ValidationFailedException ex)
