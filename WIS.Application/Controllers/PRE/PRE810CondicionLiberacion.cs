...
{
    public class PRE810CondicionLiberacion : AppController
    {
        ...
        public PRE810CondicionLiberacion(
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            try
            {
                ...
                ColaDeTrabajoPonderadorCondLiberacionQuery dbQuery = new ColaDeTrabajoPonderadorCondLiberacionQuery(nuColaDeTrabajo);
                uow.HandleQuery(dbQuery);

                var defaultSort = new SortCommand("CD_CONDICION_LIBERACION", SortDirection.Ascending);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            ColaDeTrabajoPonderadorCondLiberacionQuery query;
            string strColaDeTrabajo = context.GetParameter("nuColaDeTrabajo");

            if (int.TryParse(strColaDeTrabajo, out int nuColaDeTrabajo))
            {
                query = new ColaDeTrabajoPonderadorCondLiberacionQuery(nuColaDeTrabajo);
        ...
        public override byte[] GridExportExcel(Grid grid, GridExportExcelContext context)
        {
            ...
            var query = new ColaDeTrabajoPonderadorCondLiberacionQuery(nuColaDeTrabajo);
            ...
            try
            {
                if (grid.Rows.Any())
                {
                    foreach (var row in grid.Rows)
                    {
                        ...
                        logicCola.EditarPonderadorDetalle(nuColaDeTrabajo, int.Parse(row.GetCell("NU_PONDERACION").Value), ColasTrabajoDb.CondicionLiberacion, ColasTrabajoDb.OperacionIgual, row.GetCell("CD_CONDICION_LIBERACION").Value);
                    }
                }
