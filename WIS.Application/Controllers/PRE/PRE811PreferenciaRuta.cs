...
{
    public class PRE811PreferenciaRuta : AppController
    {
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            try
            {
                ...
                if (grid.Id == "AgregarRuta_grid_1")
                {
                    var dbQuery = new AgregarRutaPreferenciaQuery(int.Parse(codigoPreferencia));
                ...
                else if (grid.Id == "QuitarRuta_grid_2")
                {
                    if (!string.IsNullOrEmpty(codigoPreferencia))
                    {
                        ...
                        var dbQuery = new QuitarRutaPreferenciaQuery(preferencia.NU_PREFERENCIA);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            if (!string.IsNullOrEmpty(codigoPreferencia))
            {
                ...
                {
                    var dbQuery = new QuitarRutaPreferenciaQuery(int.Parse(codigoPreferencia));
                    uow.HandleQuery(dbQuery);
                    dbQuery.ApplyFilter(this._filterInterpreter, context.Filters);
        ...
        public override byte[] GridExportExcel(Grid grid, GridExportExcelContext context)
        {
            ...
            if (grid.Id == "AgregarRuta_grid_1")
            {
                var dbQuery = new AgregarRutaPreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
            ...
            {
                var dbQuery = new QuitarRutaPreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
        ...
        {
            if (context.GridId == "AgregarRuta_grid_1" && context.ButtonId == "btnAgregar")
            {
                this.ProcesarAgregar(context);
            }
            else if (context.GridId == "QuitarRuta_grid_2" && context.ButtonId == "btnQuitar")
            {
        ...
        public virtual void ProcesarAgregar(GridMenuItemActionContext context)
        {
            ...
            try
            {
                ...
                List<PreferenciaAsociarRuta> rutas = new List<PreferenciaAsociarRuta>();
                ...
                if (context.Selection.AllSelected)
                    rutas = dbQuery.GetSelectedKeysAndExclude(context.Selection.Keys, nuPreferencia);
                else
                    rutas = dbQuery.GetSelectedKeys(context.Selection.Keys, nuPreferencia);

                uow.PreferenciaRepository.AddRutasPreferencia(rutas);
                uow.SaveChanges();
                uow.Commit();
            }
            catch (ValidationFailedException ex)
