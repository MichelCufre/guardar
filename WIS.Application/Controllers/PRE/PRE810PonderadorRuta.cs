...
{
    public class PRE810PonderadorRuta : AppController
    {
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            try
            {
                ...
                var query = new ColaDeTrabajoPonderadorRutaQuery(nuColaDeTrabajo);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            ColaDeTrabajoPonderadorRutaQuery query;
            string strColaDeTrabajo = context.GetParameter("nuColaDeTrabajo");

            if (int.TryParse(strColaDeTrabajo, out int nuColaDeTrabajo))
            {
                query = new ColaDeTrabajoPonderadorRutaQuery(nuColaDeTrabajo);
            ...
            try
            {
                if (grid.Rows.Any())
                {
                    foreach (var row in grid.Rows)
                    {
                        ...
                        logicCola.EditarPonderadorDetalle(nuColaDeTrabajo, int.Parse(row.GetCell("NU_PONDERACION").Value), ColasTrabajoDb.CondicionRota, ColasTrabajoDb.OperacionIgual, row.GetCell("CD_ROTA").Value);
                    }
                }
