...
{
    public class PRE810PonderadorEmpresa : AppController
    {
        ...
        public PRE810PonderadorEmpresa(
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            try
            {
                ...
                var query = new ColaDeTrabajoPonderadorEmpresaQuery(nuColaDeTrabajo);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            ColaDeTrabajoPonderadorEmpresaQuery query;
            string strColaDeTrabajo = context.GetParameter("nuColaDeTrabajo");

            if (int.TryParse(strColaDeTrabajo, out int nuColaDeTrabajo))
            {
                query = new ColaDeTrabajoPonderadorEmpresaQuery(nuColaDeTrabajo);
            ...
            try
            {
                if (grid.Rows.Any())
                {
                    foreach (var row in grid.Rows)
                    {
                        ...
                        logicCola.EditarPonderadorDetalle(nuColaDeTrabajo, int.Parse(row.GetCell("NU_PONDERACION").Value), ColasTrabajoDb.CondicionEmpresa, ColasTrabajoDb.OperacionIgual, row.GetCell("CD_EMPRESA").Value);
                    }
                }
