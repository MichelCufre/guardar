...
{
    public class PRE811PreferenciaZona : AppController
    {
            ...
            IIdentityService identity)
        {
            ...
            {
                new SortCommand("CD_ZONA",SortDirection.Descending)
            };
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            try
            {
                ...
                if (grid.Id == "AgregarZona_grid_1")
                {
                    var dbQuery = new AgregarZonaPreferenciaQuery(int.Parse(codigoPreferencia));
                ...
                else if (grid.Id == "QuitarZona_grid_2")
                {
                    if (!string.IsNullOrEmpty(codigoPreferencia))
                    {
                        ...
                        var dbQuery = new QuitarZonaPreferenciaQuery(preferencia.NU_PREFERENCIA);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            if (!string.IsNullOrEmpty(codigoPreferencia))
            {
                ...
                {
                    var dbQuery = new QuitarZonaPreferenciaQuery(int.Parse(codigoPreferencia));
                    uow.HandleQuery(dbQuery);
                    dbQuery.ApplyFilter(this._filterInterpreter, context.Filters);
        ...
        public override byte[] GridExportExcel(Grid grid, GridExportExcelContext context)
        {
            ...
            if (grid.Id == "AgregarZona_grid_1")
            {
                var dbQuery = new AgregarZonaPreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
            ...
            {
                var dbQuery = new QuitarZonaPreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
        ...
        {
            if (context.GridId == "AgregarZona_grid_1" && context.ButtonId == "btnAgregar")
            {
                this.ProcesarAgregar(context);
            }
            else if (context.GridId == "QuitarZona_grid_2" && context.ButtonId == "btnQuitar")
            {
        ...
        public virtual void ProcesarAgregar(GridMenuItemActionContext context)
        {
            ...
            try
            {
                ...
                List<PreferenciaAsociarZona> zonas = new List<PreferenciaAsociarZona>();
                ...
                if (context.Selection.AllSelected)
                    zonas = dbQuery.GetSelectedKeysAndExclude(context.Selection.Keys, nuPreferencia);
                else
                    zonas = dbQuery.GetSelectedKeys(context.Selection.Keys, nuPreferencia);

                uow.PreferenciaRepository.AddZonasPreferencia(zonas);
                uow.SaveChanges();
                uow.Commit();
            }
            catch (ValidationFailedException ex)
