...
{
    public class PRE110AnulacionDetallePedidoLpnPendientes : AppController
    {
        ...
        protected readonly ILogger<PRE110AnulacionDetallePedidoLpnPendientes> _logger;
        ...
        public PRE110AnulacionDetallePedidoLpnPendientes(
            IIdentityService identity,
            ILogger<PRE110AnulacionDetallePedidoLpnPendientes> logger,
            ...
            ITaskQueueService taskQueue)
        {
            ...
            {
               "NU_PEDIDO", "CD_CLIENTE", "CD_EMPRESA", "CD_PRODUTO", "CD_FAIXA","NU_IDENTIFICADOR", "ID_ESPECIFICA_IDENTIFICADOR","TP_LPN_TIPO","ID_LPN_EXTERNO"
            };
        ...
        public override Grid GridInitialize(Grid grid, GridInitializeContext context)
        {
            ...
            /* grid.AddOrUpdateColumn(new GridColumnButton("BTN_ARRAY", new List<GridButton>
             {
                 new GridButton("BtnDetalles", "PRE110_grid1_btn_Atributos", "fas fa-list"),
             }));*/
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            var dbQuery = new EliminarPedidosLpnPendientesQuery();
            ...
            {
                var cantidadPendienteAtributo = decimal.Parse(row.GetCell("QT_PENDIENTE_ATRIB").Value, this._identity.GetFormatProvider());
                if (cantidadPendienteAtributo == 0)
        ...
        public override GridMenuItemActionContext GridMenuItemAction(GridMenuItemActionContext context)
        {
            ...
            uow.CreateTransactionNumber("PRE110 - Anulación Detalle de Pedido LPN");
            ...
            try
            {
                ...
                foreach (var detPedido in GetSelectedLineasPedidos(uow, context))
                {
                    ...
                    var faixa = decimal.Parse(detPedido[4], this._identity.GetFormatProvider());
                    ...
                    var detallePedidoLpn = uow.ManejoLpnRepository.GetDetallePedidoLpn(pedidoId, cliente, empresa, producto, faixa, identificador, idEspecificaIdentificador, tipoLpn, idExterno);

                    var pedidoAnulado = AnularDetallePedidoLpnTotal(uow, pedido, detallePedidoLpn);
            ...
            try
            {
                ...
                if (grid.Rows.Any())
                {
                    ...
                    foreach (var row in grid.Rows)
                    {
                        ...
                        if (row.IsDeleted)
                            pedidoAnulado = AnularDetallePedidoLpnTotal(uow, pedido, detallePedidoLpn);
                        else if (row.IsModified)
                        {
                            var cantidadAnular = decimal.Parse(row.GetCell("AUXQT_ANULADO").Value, _identity.GetFormatProvider());
                            var motivo = string.IsNullOrEmpty(row.GetCell("AUXDS_MOTIVO").Value) ? "Anulación parcial Detalle de Pedido de LPN" : row.GetCell("AUXDS_MOTIVO").Value;

                            pedidoAnulado = AnularDetallePedidoLpnParcial(uow, pedido, detallePedidoLpn, motivo, cantidadAnular);
                        }
        ...
        public override Grid GridValidateRow(GridRow row, Grid grid, GridValidationContext context)
        {
            ...
            return this._gridValidationService.Validate(new MantenimientoEliminacionPedidosPendienteLpnValidationModule(uow, this._identity.GetFormatProvider()), grid, row, context);
        }

        #region Metodo Auxiliares

        public virtual PedidoAnulado AnularDetallePedidoLpnTotal(IUnitOfWork uow, Pedido pedido, DetallePedidoLpn detalle)
        {
            var anulacionPedido = new AnulacionPedidoPendienteLpn(uow, pedido, detalle, "Anulación total de Detalle de Pedido de LPN", this._identity.UserId, this._identity.Application);

            return anulacionPedido.Anular();
        }

        public virtual PedidoAnulado AnularDetallePedidoLpnParcial(IUnitOfWork uow, Pedido pedido, DetallePedidoLpn detalle, string motivo, decimal cantidadAnular)
        {
            var anulacionPedido = new AnulacionPedidoPendienteLpn(uow, pedido, detalle, motivo, this._identity.UserId, this._identity.Application);
