...
{
    public class PRE810PonderadorExpedicion : AppController
    {
        ...
        public PRE810PonderadorExpedicion(
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            try
            {
                ...
                var query = new ColaDeTrabajoPonderadorTipoExpedidoQuery(nuColaDeTrabajo);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            ColaDeTrabajoPonderadorTipoExpedidoQuery query;
            string strColaDeTrabajo = context.GetParameter("nuColaDeTrabajo");

            if (int.TryParse(strColaDeTrabajo, out int nuColaDeTrabajo))
            {
                query = new ColaDeTrabajoPonderadorTipoExpedidoQuery(nuColaDeTrabajo);
            ...
            try
            {
                if (grid.Rows.Any())
                {
                    foreach (var row in grid.Rows)
                    {
                        ...
                        logicCola.EditarPonderadorDetalle(nuColaDeTrabajo, int.Parse(row.GetCell("NU_PONDERACION").Value), ColasTrabajoDb.CondicionExpedicion, ColasTrabajoDb.OperacionIgual, row.GetCell("TP_EXPEDICION").Value);
                    }
                }
