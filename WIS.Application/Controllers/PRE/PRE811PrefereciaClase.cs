...
{
    public class PRE811PrefereciaClase : AppController
    {
            ...
            IIdentityService identity)
        {
            ...
            {
                new SortCommand("CD_CLASSE",SortDirection.Descending)
            };
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            ...
            try
            {
                if (grid.Id == "AgregarClasse_grid_1")
                {
                    var dbQuery = new AgregarClassePreferenciaQuery(int.Parse(codigoPreferencia));
                ...
                else if (grid.Id == "QuitarClasse_grid_2")
                {
                    if (!string.IsNullOrEmpty(codigoPreferencia))
                    {
                        ...
                        var dbQuery = new QuitarClassePreferenciaQuery(preferencia.NU_PREFERENCIA);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            if (!string.IsNullOrEmpty(codigoPreferencia))
            {
                ...
                {
                    var dbQuery = new QuitarClassePreferenciaQuery(int.Parse(codigoPreferencia));
                    uow.HandleQuery(dbQuery);
                    dbQuery.ApplyFilter(this._filterInterpreter, context.Filters);
        ...
        public override byte[] GridExportExcel(Grid grid, GridExportExcelContext context)
        {
            ...
            if (grid.Id == "AgregarClasse_grid_1")
            {
                var dbQuery = new AgregarClassePreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
            ...
            {
                var dbQuery = new QuitarClassePreferenciaQuery(nuPreferencia);
                uow.HandleQuery(dbQuery);
        ...
        {
            if (context.GridId == "AgregarClasse_grid_1" && context.ButtonId == "btnAgregar")
            {
                this.ProcesarAgregar(context);
            }
            else if (context.GridId == "QuitarClasse_grid_2" && context.ButtonId == "btnQuitar")
            {
        ...
        public virtual void ProcesarAgregar(GridMenuItemActionContext context)
        {
            ...
            try
            {
                ...
                List<PreferenciaAsociarClase> classes = new List<PreferenciaAsociarClase>();
                ...
                if (context.Selection.AllSelected)
                    classes = dbQuery.GetSelectedKeysAndExclude(context.Selection.Keys, nuPreferencia);
                else
                    classes = dbQuery.GetSelectedKeys(context.Selection.Keys, nuPreferencia);

                uow.PreferenciaRepository.AddClassePreferencia(classes);
                uow.SaveChanges();
                uow.Commit();
            }
            catch (ValidationFailedException ex)
        ...
        public virtual void ProcesarQuitar(GridMenuItemActionContext context)
        {
            ...
            try
            {
                ...
                uow.PreferenciaRepository.RemoveClassePreferencia(classes);
                uow.SaveChanges();
                uow.Commit();
            }
            catch (ValidationFailedException ex)
