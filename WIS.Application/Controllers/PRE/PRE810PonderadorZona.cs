...
{
    public class PRE810PonderadorZona : AppController
    {
        ...
        public override Grid GridFetchRows(Grid grid, GridFetchContext context)
        {
            try
            {
                ...
                var query = new ColaDeTrabajoPonderadorZonaQuery(nuColaDeTrabajo);
        ...
        public override GridStats GridFetchStats(Grid grid, GridFetchStatsContext context)
        {
            ...
            using var uow = this._uowFactory.GetUnitOfWork();
            ColaDeTrabajoPonderadorZonaQuery query;
            string strColaDeTrabajo = context.GetParameter("nuColaDeTrabajo");

            if (int.TryParse(strColaDeTrabajo, out int nuColaDeTrabajo))
            {
                query = new ColaDeTrabajoPonderadorZonaQuery(nuColaDeTrabajo);
            ...
            try
            {
                if (grid.Rows.Any())
                {
                    foreach (var row in grid.Rows)
                    {
                        ...
                        logicCola.EditarPonderadorDetalle(nuColaDeTrabajo, int.Parse(row.GetCell("NU_PONDERACION").Value), ColasTrabajoDb.CondicionZona, ColasTrabajoDb.OperacionIgual, row.GetCell("CD_ZONA").Value);
                    }
                }
