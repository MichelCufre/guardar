...
{
    public class IMP050ImpresionUsuarios : AppController
    {
        ...
        public IMP050ImpresionUsuarios(IIdentityService identity,
        ...
        public override Form FormSubmit(Form form, FormSubmitContext context)
        {
            ...
            List<string> FuncionariosParaImprimir = JsonConvert.DeserializeObject<List<string>>(context.Parameters.FirstOrDefault(x => x.Id == "ListaFilasSeleccionadas")?.Value);
            ...
            string clavesFuncionarioReferencia = string.Empty;

            uow.CreateTransactionNumber(this._identity.Application);

            foreach (var func in FuncionariosParaImprimir)
            {
                funcionarios.Add(uow.FuncionarioRepository.GetFuncionario(int.Parse(func)));
            }

            IEstiloTemplate estiloTemplate = new EstiloTemplate(uow, estilo);
            IImpresionDetalleBuildingStrategy strategy = new FuncionarioImpresionStrategy(estiloTemplate, _printingService, funcionarios);
            ...
            if (funcionarios.Count > 1)
                impresion.Referencia = string.Format("Cant. Func: {0}", funcionarios.Count());
            else
                impresion.Referencia = string.Format("Funcio: {0}", funcionarios.FirstOrDefault().NombreLogin);
        ...
        public override Form FormValidateForm(Form form, FormValidationContext context)
        {
            ...
            return this._formValidationService.Validate(new ImpresionesUsuarioValidationModule(uow, this._identity.UserId, this._identity.Predio), form, context);
        }
        ...
        public virtual void InicializarSelect(IUnitOfWork uow, Form form)
        {
            ...
            if (selectorImpresora.Value != "")
            {
                ...
                List<EtiquetaEstiloLenguaje> listaEstilos = uow.ImpresionRepository.GetEstiloByLenguaje(lenguaje, EstiloEtiquetaDb.Funcionario);
            ...
            {
                List<EtiquetaEstiloLenguaje> listaEstilos = uow.ImpresionRepository.GetEstiloByTipo(EstiloEtiquetaDb.Funcionario);
                foreach (var estilo in listaEstilos)
                {
                    selectorEstilo.Options.Add(new SelectOption(estilo.CodigoLabel, $"{estilo.CodigoLabel} - {estilo.Descripcion}"));
                }
