...
{
    public class IMP080ImpresionRecepciones : AppController
    {
        ...
        public IMP080ImpresionRecepciones(IIdentityService identity,
        ...
        public override Form FormInitialize(Form form, FormInitializeContext context)
        {
            ...
            form.GetField("numCopias").Value = "1";
            form.GetField("numCopias").ReadOnly = true;
        ...
        public override Form FormSubmit(Form form, FormSubmitContext context)
        {
            ...
            List<string> etiquetasRecepcionParaImprimir = JsonConvert.DeserializeObject<List<string>>(context.Parameters.FirstOrDefault(x => x.Id == "ListaFilasSeleccionadas")?.Value);
            ...
            List<EtiquetaLote> etiquetas = new List<EtiquetaLote>();
            string clavesEtiquetasReferencia = string.Empty;

            uow.CreateTransactionNumber(this._identity.Application);

            foreach (var etiq in etiquetasRecepcionParaImprimir)
            {
                var stringEtiquetaClave = etiq.Split('$');

                int etiquetaLote = int.Parse(stringEtiquetaClave[0]);
                int empresa = int.Parse(stringEtiquetaClave[1]);
                int agenda = int.Parse(stringEtiquetaClave[2]);

                etiquetas.Add(uow.EtiquetaLoteRepository.GetEtiquetaLote(etiquetaLote));

                if (etiq == etiquetasRecepcionParaImprimir.Last())
                    clavesEtiquetasReferencia += etiquetaLote.ToString();
                else
                    clavesEtiquetasReferencia += etiquetaLote.ToString() + " - ";
            }
            ...
            {
                etiq.TipoEtiqueta = EstiloEtiquetaDb.Recepcion;
            }

            IEstiloTemplate estiloTemplate = new EstiloTemplate(uow, estilo);
            IImpresionDetalleBuildingStrategy strategy = new EtiquetaRecepcionImpresionStrategy(estiloTemplate, etiquetas, uow, _printingService, _barcodeService);
            ...
            if (etiquetas.Count == 1)
                impresion.Referencia = string.Format("Impresión de etiqueta de {0} número {1}", etiquetas.First().TipoEtiqueta, etiquetas.First().Numero.ToString());
            else
                impresion.Referencia = string.Format("Impresión de {0} etiquetas, números: {1}", etiquetas.Count.ToString(), clavesEtiquetasReferencia);
        ...
        public override Form FormValidateForm(Form form, FormValidationContext context)
        {
            ...
            return this._formValidationService.Validate(new ImpresionImpresionEtiquetasRecepcionValidationModule(uow, this._identity), form, context);
        }
