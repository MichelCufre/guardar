...
namespace WIS.MiddlewareAPI.Controllers.Entrada
{
    ...
    public class PedidoController : MiddlewareControllerBase
    {
        public PedidoController(IUnitOfWorkFactory uowFactory, ILogger<PedidoController> logger)
            : base(uowFactory, logger) { }

        /// <summary>Encola la creacion o actualizacion de pedidos.</summary>
        ...
        public IActionResult CreateOrUpdate([FromBody] ErpPedidosRequest request)
        {
            var wmsRequest = ErpMapper.ToWms(request);
            return Encolar(MiddlewareColaTipo.Pedido, wmsRequest);
        }
    }
}
