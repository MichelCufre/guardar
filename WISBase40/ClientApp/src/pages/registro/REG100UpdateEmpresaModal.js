...
import { Form, Field, FieldSelect, FieldCheckbox, FieldSelectAsync, FieldTextArea, StatusMessage, SubmitButton } from '../../components/FormComponents/Form';
import * as Yup from 'yup';
import { withPageContext } from '../../components/WithPageContext';
import $ from 'jquery';
import './REG100.css';

function InternalREG100UpdateEmpresaModal(props) {
    const { t } = useCustomTranslation();

    const [mostrarCamposWebhook, setMostrarCamposWebhook] = useState(false);
    const [mostrarCamposSecret, setMostrarCamposSecret] = useState(false);
    const [campoSecretCambiable, setCampoSecretCambiable] = useState(false);
    ...
    const validationSchema = {
        ...
        secret: Yup.string(),
        resecret: Yup.string(),
        habilitadoCambioSecret: Yup.string()
    };
    ...
    const handleFormAfterInitialize = (context, form, query, nexus) => {
        var isWebhook = form.fields.find(f => f.id === "tipoNotificacion").value === query.parameters.find(p => p.id === "tipoNotificacionWebhook").value;
        setMostrarCamposWebhook(isWebhook);
        setCampoSecretCambiable(isWebhook);
        setMostrarCamposSecret(false);
    };

    const handleFormBeforeValidateField = (context, form, query, nexus) => {
        var mostrarCamposSecret = form.fields.find(f => f.id === "habilitadoCambioSecret").value;
        setMostrarCamposSecret(mostrarCamposSecret);
        query.parameters = [
            { id: "keyEmpresa", value: props.agente.find(a => a.id === "IdEmpresa").value },
            { id: "mostrarCamposSecret", value: mostrarCamposSecret ? "true" : "false" },
            { id: "mostrarCamposWebhook", value: mostrarCamposWebhook ? "true" : "false" }
        ];
    }

    const handleFormAfterValidateField = (context, form, query, nexus) => {
        if (query.parameters.find(x => x.id === "mostrarCamposSecret") != null) {
            setMostrarCamposSecret(query.parameters.find(x => x.id === "mostrarCamposSecret").value == "true");
        }

        var isWebhook = false;

        if (query.parameters.find(x => x.id === "mostrarCamposWebhook") != null) {
            isWebhook = query.parameters.find(x => x.id === "mostrarCamposWebhook").value == "true";
            setMostrarCamposWebhook(isWebhook);
        }

        if (!campoSecretCambiable && isWebhook) {
            setMostrarCamposSecret(true);
        }
    }
    ...
    const handleFormBeforeSubmit = (context, form, query, nexus) => {
        ...
        query.parameters = [
            ...
            { id: "mostrarCamposSecret", value: mostrarCamposSecret },
            { id: "mostrarCamposWebhook", value: mostrarCamposWebhook }
        ];
    ...
    const testWebhook = () => {
        ...
        const secret = mostrarCamposSecret ? $("[name='secret']").val() : null;
        const empresa = $("[name='codigoEmpresa']").val();
        const request = {
            method: 'POST',
            cache: "no-cache",
            headers: {
        ...
        <Modal show={props.show} onHide={handleClose} dialogClassName="modal-90w" backdrop="static">
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title>{t("REG100_Sec0_mdlUpdate_Titulo")}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Row>
                        <Col>
                                ...
                                <Tab eventKey="notifications" title={t("REG100_frm1_tab_notifications")}>
                                    ...
                                    <Row style={{ display: mostrarCamposWebhook ? 'block' : 'none' }}>
                                        <Col>
                                            ...
                                            <div className="form-group" style={{ display: campoSecretCambiable ? 'block' : 'none' }} >
                                                <label htmlFor="habilitadoCambioSecret">{t("REG100_frm1_lbl_changeSecret")}</label>
                                                <FieldCheckbox name="habilitadoCambioSecret" />
                                                <StatusMessage for="habilitadoCambioSecret" />
                                            </div>
                                            <div style={{ display: mostrarCamposSecret ? 'block' : 'none' }}>
                                                <div className="form-group" >
                ...
                <Modal.Footer>
                    <Button variant="btn btn-outline-secondary" onClick={handleClose}>
                        {t("REG100_frm1_btn_CANCELAR")}
                    </Button>
                    <SubmitButton id="btnSubmitUpdateEmpresa" variant="primary" label="REG100_frm1_btn_MODIFICAR" />
                </Modal.Footer>
            </Form>
        </Modal>
    );
...
export const REG100UpdateEmpresaModal = withPageContext(InternalREG100UpdateEmpresaModal);