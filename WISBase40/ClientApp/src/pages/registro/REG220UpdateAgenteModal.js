...
function InternalREG220UpdateAgenteModal(props) {
    ...
    const handleFormAfterInitialize = (context, form, query, nexus) => {
        ...
        const recurso = form.fields.find(a => a.id == "tipoAgente").value;
        form.fields.find(a => a.id == "tipoAgente").value = t(recurso);

    };

    const handleFormBeforeValidateField = (context, form, query, nexus) => {
        query.parameters.push({ id: "keyEmpresa", value: props.agente.find(a => a.id === "IdEmpresa").value });
    }
    ...
    const handleFormAfterValidateField = (context, form, query, nexus) => {
        ...
        const formComponent = nexus.getForm("REG220Update_form_1");
        ...
        <Modal show={props.show} onHide={handleClose} dialogClassName="modal-90w" backdrop="static">
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title>{t("REG220_Sec0_mdlUpdate_Titulo")}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Row>
                        <Col>
                ...
                <Modal.Footer>
                    ...
                    <SubmitButton id="btnSubmitCreateFamilia" variant="primary" label="REG220_frm1_btn_MODIFICAR" />
                </Modal.Footer>
            </Form>
        </Modal>
    );
...
export const REG220UpdateAgenteModal = withPageContext(InternalREG220UpdateAgenteModal);