...
export function InternalPRE810EditarPonderadorFechaLiberado(props) {
    ...
    const GridOnAfterButtonAction = (data, nexus) => {
        nexus.getForm("PRE810FechaLiberado_form_1").reset(data.parameters);
    };

    const onAfterCommit = (context, rows, parameters, nexus) => {
        nexus.getGrid("PRE810PonderadorFechaLib_grid_1").refresh();
    }
    ...
    const onAfterSubmit = (context, form, query, nexus) => {
        if (context.status === "ERROR")
            return false;
        nexus.getGrid("PRE810PonderadorFechaLib_grid_1").reset();
        nexus.getForm("PRE810FechaLiberado_form_1").reset();
    };
    ...
    const onAfterValidateField = (context, form, query, nexus) => {
        ...
        if (operador) {
            if (operador.value != "entre") {
                ...
                $("#labelHoras").show();
                $("#horas").show();
                nexus.getForm("PRE810FechaLiberado_form_1").setFieldValue("diasHasta", "");
                nexus.getForm("PRE810FechaLiberado_form_1").setFieldValue("horasHasta", "");
            }
            else {
                $("#labelDias").hide();
                $("#labelHoras").hide();
    ...
    const handleCancel = () => {
        $('#PRE810FechaLiberado_form_1').trigger("reset");
    };

    return (
        <Modal dialogClassName="modal-50w" show={props.show} onHide={props.onHide} >
            <Modal.Header closeButton>
                <Modal.Title>{t("PRE810_Sec0_lbl_Title_DT_LIBERADO")}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Page
                    load
                ...
                >
                    <Row>
                        <Col>
                            <Form
                                id="PRE810FechaLiberado_form_1"
                                application="PRE810PonderadorFechaLib"
                    ...
                    <br />
                    <Grid
                        id="PRE810PonderadorFechaLib_grid_1"
                        application="PRE810PonderadorFechaLib"
                        rowsToFetch={30}
                        rowsToDisplay={10}
...
export const PRE810EditarPonderadorFechaLiberado = withPageContext(InternalPRE810EditarPonderadorFechaLiberado);