...
function InternalPRE811FamiliaModal(props) {
    ...
    const handleAfterMenuItemAction = (context, data, nexus) => {
        nexus.getGrid("AgregarFamilia_grid_1").refresh();
        nexus.getGrid("QuitarFamilia_grid_2").refresh();
    }

    const handleAdd = (evt, nexus) => {
        nexus.getGrid("AgregarFamilia_grid_1").triggerMenuAction("btnAgregar", false, evt.ctrlKey);
    }

    const handleRemove = (evt, nexus) => {
        nexus.getGrid("QuitarFamilia_grid_2").triggerMenuAction("btnQuitar", false, evt.ctrlKey);
    }
    ...
    return (
        <Modal show={props.show} onHide={handleClose} dialogClassName="modal-90w" backdrop="static">
            <Modal.Header closeButton>
                <Modal.Title>{t("PRE811Fam_Sec0_modal_title")}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Page
                    {...props}
                    onBeforeLoad={handleBeforeLoad}
                    application="PRE811PreferenciaFamilia"
                >
                    <Row>
                        <Col>
...
export const PRE811FamiliaModal = withPageContext(InternalPRE811FamiliaModal);