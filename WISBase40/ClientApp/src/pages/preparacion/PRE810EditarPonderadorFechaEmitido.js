...
import { Button, Col, Modal, Row, FormGroup } from 'react-bootstrap';
import { useTranslation } from 'react-i18next';
import $ from 'jquery';
import * as Yup from 'yup';
import { Grid } from '../../components/GridComponents/Grid';
import { Page } from '../../components/Page';
import { Field, FieldNumber, FieldSelect, FieldSelectAsync, Form, StatusMessage, SubmitButton } from '../../components/FormComponents/Form';
import { withPageContext } from '../../components/WithPageContext';

export function InternalPRE810EditarPonderadorFechaEmitido(props) {
    ...
    const GridOnAfterButtonAction = (data, nexus) => {
        nexus.getForm("PRE810FechaEmitido_form_1").reset(data.parameters);
    };

    const onAfterCommit = (context, rows, parameters, nexus) => {
        nexus.getGrid("PRE810PonderadorFechaEmitido_grid_1").refresh();
    }
    ...
    const onAfterSubmit = (context, form, query, nexus) => {
        if (context.status === "ERROR")
            return false;
        nexus.getGrid("PRE810PonderadorFechaEmitido_grid_1").reset();
        nexus.getForm("PRE810FechaEmitido_form_1").reset();
    };
    ...
    const onAfterValidateField = (context, form, query, nexus) => {
        ...
        if (operador) {
            if (operador.value != "entre") {
                ...
                $("#labelHoras").show();
                $("#horas").show();
                nexus.getForm("PRE810FechaEmitido_form_1").setFieldValue("diasHasta", "");
                nexus.getForm("PRE810FechaEmitido_form_1").setFieldValue("horasHasta", "");
            }
            else {
                $("#labelDias").hide();
                $("#labelHoras").hide();
    ...
    const handleCancel = () => {
        $('#PRE810FechaEmitido_form_1').trigger("reset");
    };

    return (
        <Modal dialogClassName="modal-50w" show={props.show} onHide={props.onHide} >
            <Modal.Header closeButton>
                <Modal.Title>{t("PRE810_Sec0_lbl_Title_DT_EMITIDO")}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Page
                    load
                ...
                >
                    <Row>
                        <Col>
                            <Form
                                id="PRE810FechaEmitido_form_1"
                                application="PRE810PonderadorFechaEmitido"
                    ...
                    <br />
                    <Grid
                        id="PRE810PonderadorFechaEmitido_grid_1"
                        application="PRE810PonderadorFechaEmitido"
                        rowsToFetch={30}
                        rowsToDisplay={10}
...
export const PRE810EditarPonderadorFechaEmitido = withPageContext(InternalPRE810EditarPonderadorFechaEmitido);