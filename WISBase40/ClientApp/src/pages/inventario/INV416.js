...
export default function INV416(props) {
    ...
    useEffect(() => {
        if (nexus) {
            nexus.getGrid('INV416_grid_1').refresh();
            nexus.getGrid('INV416_grid_2').refresh();
        }
    }, [filtro]);

    const onBeforeFetch = (context, data, nexus) => {
        if (data.rowsToSkip == 0) {
            if (data.gridId == "INV416_grid_AtrCab") {
    ...
    const onAfterApplySort = (context, rows, parameters) => {
        if (context.gridId == "INV416_grid_AtrCab") {
            setFiltro({ AtributosCabezal: [], AtributosDetalle: [] });
        }
    }
    ...
    const onBeforeButtonAction = (context, form, query, nexus) => {
        ...
        if (query.buttonId === "btnAplicarFiltro") {
            ...
            forEach(nexus.getGrid('INV416_grid_AtrCab').getModifiedRows(), row => {
    ...
    return (
        <Page
            title={t("INV416_Sec0_pageTitle_Titulo")}
            {...props}
            application="INV416"
