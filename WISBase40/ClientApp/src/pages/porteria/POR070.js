...
export default function POR070(props) {
    ...
    const formOnBeforeButtonAction = (context, form, query, nexus) => {
        ...
        if (query.buttonId === "btnConfirmar") {
            nexus.getGrid("POR070_grid_Agregar").triggerMenuAction("btnAgregar");
        }
        else if (query.buttonId === "btnVolver") {
            nexus.getGrid("POR070_grid_Agregar").clearRows();
            nexus.getGrid("POR070_grid_Quitar").clearRows();
    ...
    const onBeforeValidateField = (context, form, query, nexus) => {
        ...
        if (query.fieldId === "FL_FILTER") {
            ...
            nexus.getGrid("POR070_grid_Agregar").refresh(parameters);
    ...
    const addParameters = (context, data, nexus) => {
        ...
        if (data.parameters.length === 0) {
            ...
            data.parameters = [
                ...
                {
                    id: "FL_FILTER",
                    value: nexus.getForm("POR070_form_1").getFieldValue("FL_FILTER") || ""
                },
            ];
        }
    ...
    const onBeforeMenuItemAction = (context, data, nexus) => {
        ...
        if (!data.selection.allSelected && data.selection.keys.length == 0) {
            context.abortServerCall = true;
            if (data.gridId === "POR070_grid_Agregar") {
                nexus.getGrid("POR070_grid_Quitar").triggerMenuAction("btnQuitar");
            }
            else if (data.gridId === "POR070_grid_Quitar") {
                nexus.getGrid("POR070_grid_Agregar").clearRows();
                nexus.getGrid("POR070_grid_Quitar").clearRows();
    ...
    const gridOnBeforeButtonAction = (context, data, nexus) => {
        if (data.buttonId === "btnEditar") {
            ...
            nexus.getGrid("POR070_grid_Quitar").refresh(parameters);
    ...
    return (
        ...
        <Page
            title={t("POR070_Sec0_pageTitle_Titulo")}
            {...props}
        >
            ...
            >
                <div className={isHiddenGrid ? 'hidden' : ''}>
                    ...
                    <Row className="mb-4">
                        <Col lg="2">
                            <FormGroup>
                                <h3>{`${t("POR070_frm1_lbl_Vehiculo")} ${nuPorteriaVehiculo}`}</h3>
                            </FormGroup>
                        </Col>
                        <Col lg="4">
                            <FormGroup>
                    ...
                    <Row>
                        <Col>
                            <h3>{t("POR070_frm1_lbl_SubTituloAgregar")}</h3>
                            <Grid id="POR070_grid_Agregar" rowsToFetch={30} rowsToDisplay={10} enableSelection
                        ...
                        <Col>
                            <h3>{t("POR070_frm1_lbl_SubTituloQuitar")}</h3>
                            <Grid id="POR070_grid_Quitar" rowsToFetch={30} rowsToDisplay={10} enableSelection
                                onBeforeInitialize={addParameters}
                                onBeforeFetch={addParameters}
                                onBeforeFetchStats={addParameters}
                                onBeforeApplyFilter={addParameters}
            ...
            <div className={!isHiddenGrid ? 'hidden' : 'row mb-4'}>
                <div className="col-12">
                    <Grid id="POR070_grid_1" rowsToFetch={30} rowsToDisplay={15} enableExcelExport
