...
export function STO800AgregarEmpresasModal(props) {
    const { t } = useTranslation();

    const handleAdd = (evt, nexus) => {
        nexus.getGrid("STO800AsignarEmpresas_grid_1").triggerMenuAction("btnAgregar", false, evt.ctrlKey);
    };

    const handleRemove = (evt, nexus) => {
        nexus.getGrid("STO800AsignarEmpresas_grid_2").triggerMenuAction("btnQuitar", false, evt.ctrlKey);
    };
    ...
    const onAfterMenuItemAction = (context, data, nexus) => {
        nexus.getGrid("STO800AsignarEmpresas_grid_1").refresh();
        nexus.getGrid("STO800AsignarEmpresas_grid_2").refresh();
    ...
    return (
        <Modal show={props.show} onHide={handleClose} dialogClassName="modal-90w" backdrop="static">
            <Modal.Header closeButton>
                <Modal.Title>{t("STO800_Sec0_mdlTitle_AsigEmpresasTitulo")}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Page
                    {...props}
                    application="STO800AgregarEmpresasDestino"
                >
                    <Row>
                        <Col>
                        ...
                        <div className="col-12">
                            <AddRemovePanel
                                ...
                                from={(
                                    <Grid
                                        application="STO800AgregarEmpresasDestino"
                                        id="STO800AsignarEmpresas_grid_1" rowsToFetch={30} rowsToDisplay={15} enableExcelExport
                                        enableSelection
                                ...
                                to={(
                                    <Grid
                                        application="STO800AgregarEmpresasDestino"
                                        id="STO800AsignarEmpresas_grid_2" rowsToFetch={30} rowsToDisplay={15} enableExcelExport
                                        enableSelection
                                        onAfterMenuItemAction={onAfterMenuItemAction}
                                        onBeforeInitialize={addParameters}
                                        onBeforeFetch={addParameters}
