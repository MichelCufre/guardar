...
function InternalREC170PlanificacionLpnModal(props) {
    ...
    const handleAfterMenuItemAction = (context, data, nexus) => {
        nexus.getGrid("REC170PlanificacionLpn_grid_1").refresh();
        nexus.getGrid("REC170PlanificacionLpn_grid_2").refresh();
    }

    const handleAdd = (evt, nexus) => {
        nexus.getGrid("REC170PlanificacionLpn_grid_1").triggerMenuAction("btnAgregar", false, evt.ctrlKey);
    };

    const handleRemove = (evt, nexus) => {
        nexus.getGrid("REC170PlanificacionLpn_grid_2").triggerMenuAction("btnQuitar", false, evt.ctrlKey);
    };
    ...
    return (
        <Modal show={props.show} onHide={handleClose} dialogClassName="modal-90w" backdrop="static">
            <Modal.Header closeButton>
                <Modal.Title>{t("REC170PlanificacionLpn_Sec0_lbl_Titulo")}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Page
                    {...props}
                    application="REC170PlanificacionLpn"
                ...
                >
                    ...
                    <AddRemovePanel
                        ...
                        from={(
                            <Grid
                                application="REC170PlanificacionLpn"
                                id="REC170PlanificacionLpn_grid_1"
                                rowsToFetch={30}
                                rowsToDisplay={15}
                        ...
                        to={(
                            <Grid
                                application="REC170PlanificacionLpn"
                                id="REC170PlanificacionLpn_grid_2"
                                rowsToFetch={30}
                                rowsToDisplay={15}
                                onBeforeInitialize={applyParameters}
                                onBeforeFetch={applyParameters}
...
export const REC170PlanificacionLpnModal = withPageContext(InternalREC170PlanificacionLpnModal);
