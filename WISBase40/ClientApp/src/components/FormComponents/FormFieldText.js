...
export class InternalFieldText extends Component {
    constructor(props) {
        super(props);
    ...
    handleChange = async (event) => {
        ...
        await formik.setFieldValue(this.props.name, event.target.value, false);
    }
    ...
    render() {
        ...
        if (this.props.isHtml) {
            return <div className="content border rounded-1 form-control" style={this.getStyle()}>
                <div className="h-100 overflow-auto" dangerouslySetInnerHTML={{ __html: value }}></div>
            </div>
        } else {
            return (
                <textarea
...
export const FieldText = withFormContext(connect(InternalFieldText));