...
{
    public class ValidacionFacturacionPickingCompleto : IFacturacionValidacion
    {
        ...
        public ValidacionFacturacionPickingCompleto(IUnitOfWork uow, IValidacionFacturacionResultFormatResolver resolver)
        {
            this._uow = uow;
            this._resultResolver = resolver;
            this.PedidosError = new List<Pedido>();
            this.Message = "Existen lineas sin pickear";
        }

        public virtual void Validate(Camion camion, Pedido pedido)
        {
            if (this._uow.PreparacionRepository.AnyLineaSinPickear(pedido.Empresa, pedido.Cliente, pedido.Id))
