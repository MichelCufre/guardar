...
{
    public class StockEtiquetaRecepcionQuery : QueryObject<V_ETIQUETAS_WREC150, WISDB>
    {
        ...
        public StockEtiquetaRecepcionQuery(int empresa, string ubicacion, string producto, decimal faixa, string identificador, decimal qtEntrada)
        {
        ...
        {
            this.Query = context.V_ETIQUETAS_WREC150.Where(d => (d.CD_SITUACAO == SituacionDb.PalletConferido || d.CD_SITUACAO == SituacionDb.PalletGeneradoAutomaticamente)
            ...
            if (this._qtEntrada > 0)
                this.Query = this.Query.Where(d => d.CD_ENDERECO_SUGERIDO == this._ubicacion);
            else
