...
{
    public class PedidosLpnAtributosPendientesQuery : QueryObject<V_PRE110_ATRIBUTOS_LPN_DEFINIDOS, WISDB>
    {
        ...
        public PedidosLpnAtributosPendientesQuery(string pedido, int empresa, string cliente, string idEspecificaIdentificador, string idLpnExteno, string lpnTipo, string producto, string identificador, decimal faixa, long nuDetPedSaiAtrib)
        {
            this._pedido = pedido;
        ...
        {
            this.Query = context.V_PRE110_ATRIBUTOS_LPN_DEFINIDOS.AsNoTracking();
            ...
            this.Query = this.Query.Where(x => x.NU_PEDIDO == _pedido && x.CD_EMPRESA == _empresa && x.CD_CLIENTE == _cliente && x.ID_ESPECIFICA_IDENTIFICADOR == _idEspecificaIdentificador
                                                && x.ID_LPN_EXTERNO == _idLpnExteno && x.TP_LPN_TIPO == _lpnTipo && x.CD_PRODUTO == _producto && x.NU_IDENTIFICADOR == _identificador && x.CD_FAIXA == _faixa && x.NU_DET_PED_SAI_ATRIB == _nuDetPedSaiAtrib);
