...
{
    public class ControlCambioReportSetup : IReportSetup
    {
        ...
        protected readonly string _tipoReporte = CReporte.CONTROL_CAMBIO;

        public ControlCambioReportSetup(IUnitOfWork uow, IParameterService parameterService, IReportKeyService keyResolver, IIdentityService securityService, Camion camion, string predio = null)
        {
            this._uow = uow;
        ...
        public ControlCambioReportSetup(IUnitOfWork uow)
        {
        ...
        public virtual void GenerarReporte(IUnitOfWork uow, DbConnection connection, ILogger<ReportLogic> logger, DtoReporteRelacion relacion)
        {
            ...
            {
                logger.LogDebug(ex, $"GenerarReporteControlCambio - Clave: " + relacion.Clave);
        ...
        public virtual void Generar(IUnitOfWork uow, DbConnection connection, int cdCamion, int cdEmpresa, string cdCliente, string language, out byte[] fileData, out string fileName)
        {
            ...
            var datos = uow.ReporteRepository.GetInfoControlCambio(connection, cdCamion);
            var detalles = uow.ReporteRepository.GetInfoControlCambioDet(connection, cdCamion, cdEmpresa, cdCliente);
            var detallesLpn = uow.ReporteRepository.GetInfoControlCambioLpn(connection, cdCamion);
            ...
            {
                "WCOF070_ReporteConCon_lbl_CD_CLIENTE",
                "WCOF070_ReporteConCon_lbl_CD_CAMION",
                "WCOF070_ReporteConCon_lbl_CD_PLACA_CARRO",
                "WCOF070_ReporteConCon_lbl_CD_TRANSPORTADORA",
                "WCOF070_ReporteConCon_lbl_DT_CIERRE",
                "WCOF070_ReporteConCon_lbl_CD_EMPRESA",
                "WCOF070_ReporteConCon_lbl_Pagina",
                "WCOF070_ReporteConCon_lbl_SeparadorPagina",
                "WCOF070_ReporteConCon_lbl_DS_TIPO_CONTENEDOR",
                "WCOF070_ReporteConCon_lbl_NU_CONTENEDOR",
                "WCOF070_ReporteConCon_lbl_QT_BULTO",
                "WCOF070_ReporteConCon_lbl_QT_TOTAL",
                "WCOF070_ReporteConCon_lbl_ID_EXTERNO_CONTENEDOR"
            }, language);
        ...
        public virtual void CreateHeader(Document doc, List<TranslatedValue> resourceList, DtoReporteControlCambio datos, DtoReporteControlCambio datosCliente, string filePath, bool reporteConLogo)
        {
            if (reporteConLogo)
            {
            ...
            var head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_CD_CAMION")?.Value ?? "Camión").SetFontColor(ColorConstants.BLACK).SetPaddingLeft(10));
            ...
            head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_CD_CLIENTE")?.Value ?? "Agente").SetFontColor(ColorConstants.BLACK).SetPaddingLeft(10));
            head.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
            head.SetBackgroundColor(ColorConstants.LIGHT_GRAY);
            table.AddCell(head);
            ...
            head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_CD_PLACA_CARRO")?.Value ?? "Matrícula").SetFontColor(ColorConstants.BLACK).SetPaddingLeft(10));
            head.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
            head.SetBackgroundColor(ColorConstants.LIGHT_GRAY);
            table.AddCell(head);
            ...
            head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_CD_EMPRESA")?.Value ?? "Empresa").SetFontColor(ColorConstants.BLACK).SetPaddingLeft(10));
            head.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
            head.SetBackgroundColor(ColorConstants.LIGHT_GRAY);
            table.AddCell(head);
            ...
            head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_DT_CIERRE")?.Value ?? "Fecha cierre").SetFontColor(ColorConstants.BLACK).SetPaddingLeft(10));
            head.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
            head.SetBackgroundColor(ColorConstants.LIGHT_GRAY);
            table.AddCell(head);
            ...
            head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_CD_TRANSPORTADORA")?.Value ?? "Transportista").SetFontColor(ColorConstants.BLACK).SetPaddingLeft(10));
            head.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
            head.SetBackgroundColor(ColorConstants.LIGHT_GRAY);
            table.AddCell(head);
        ...
        public virtual void CreateContent(Document doc, List<TranslatedValue> resourceList, List<DtoReporteControlCambio> detalles, List<DtoReporteControlCambioLpn> detallesLpn)
        {
            Cell head = new Cell();

            #region Tabla_Tipos

            var summaryData = detalles.GroupBy(d => d.DescripcionTipoContenedor).Select(d => new { DescripcionTipoContenedor = d.Key, CantidadBulto = d.Sum(e => e.CantidadBulto ?? 1) })
               .OrderBy(d => d.DescripcionTipoContenedor).ToList();

            var table = new Table(UnitValue.CreatePercentArray(2)).UseAllAvailableWidth().SetBackgroundColor(new iText.Kernel.Colors.DeviceRgb(0, 0, 255), 0).SetFontSize(9)
                .SetBorder(new SolidBorder(ColorConstants.BLACK, 1));

            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_DS_TIPO_CONTENEDOR")?.Value ?? "Tipo de Contenedor").SetFontColor(ColorConstants.BLACK).SetTextAlignment(TextAlignment.CENTER));
            ...
            head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_QT_TOTAL")?.Value ?? "Cant. de tipos").SetFontColor(ColorConstants.BLACK).SetTextAlignment(TextAlignment.CENTER));
            head.SetBorder(Border.NO_BORDER);
            head.SetBackgroundColor(ColorConstants.GRAY);

            table.AddHeaderCell(head);

            foreach (var linea in summaryData)
            {
                Cell cell = new Cell();
                cell.Add(new Paragraph(linea.DescripcionTipoContenedor));
                cell.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));

                table.AddCell(cell);

                cell = new Cell();
                cell.Add(new Paragraph(linea.CantidadBulto.ToString()));
                cell.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
            ...
            #region Tabla_Contenedores

            table = new Table(UnitValue.CreatePercentArray(3)).UseAllAvailableWidth().SetBackgroundColor(new iText.Kernel.Colors.DeviceRgb(0, 0, 255), 0).SetFontSize(9)
                .SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
            ...
            head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_ID_EXTERNO_CONTENEDOR")?.Value ?? "Id. Externo").SetFontColor(ColorConstants.BLACK).SetTextAlignment(TextAlignment.CENTER));
            head.SetBorder(Border.NO_BORDER);
            head.SetBackgroundColor(ColorConstants.GRAY);
            ...
            head = new Cell();
            head.Add(new Paragraph(resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_QT_BULTO")?.Value ?? "Cant. de Bultos").SetFontColor(ColorConstants.BLACK).SetTextAlignment(TextAlignment.CENTER));
            head.SetBorder(Border.NO_BORDER);
            head.SetBackgroundColor(ColorConstants.GRAY);
            ...
            foreach (var linea in detalles)
            {
                ...
                cell = new Cell();
                cell.Add(new Paragraph(linea.IdExternoContenedor));
                cell.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));

                table.AddCell(cell);

                cell = new Cell();
                cell.Add(new Paragraph((linea.CantidadBulto ?? 1).ToString()));
                cell.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
            ...
            if (detallesLpn != null && detallesLpn.Count() > 0)
            {
                ...
                foreach (var linea in detalles)
                {
                    var textIdExterno = resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_ID_EXTERNO_CONTENEDOR")?.Value ?? "Id. Externo";
                    var textTipoContenedor = resourceList.FirstOrDefault(x => x.Key == "WCOF070_ReporteConCon_lbl_DS_TIPO_CONTENEDOR")?.Value ?? "Tipo de Contenedor";

                    head = new Cell();
                    head.Add(new Paragraph($"{textIdExterno} : {linea.IdExternoContenedor}").SetFontColor(ColorConstants.BLACK).SetTextAlignment(TextAlignment.CENTER));
                    head.SetBorder(Border.NO_BORDER);
                    head.SetBackgroundColor(ColorConstants.GRAY);
                    ...
                    head = new Cell(1, 2);
                    head.Add(new Paragraph($"{textTipoContenedor}: {linea.DescripcionTipoContenedor}").SetFontColor(ColorConstants.BLACK).SetTextAlignment(TextAlignment.CENTER));
                    head.SetBorder(Border.NO_BORDER);
                    head.SetBackgroundColor(ColorConstants.GRAY);

                    table.AddCell(head);

                    var atributos = detallesLpn.Where(a => a.NroLpn == linea.NroLpn);
                    foreach (var atributo in atributos)
                    {
                        Cell cell = new Cell();
                        cell.Add(new Paragraph(atributo.NombreAtributo));
                        cell.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));

                        table.AddCell(cell);

                        cell = new Cell(1, 2);
                        cell.Add(new Paragraph(atributo.ValorAtributo));
                        cell.SetBorder(new SolidBorder(ColorConstants.BLACK, 1));
        ...
        public virtual void Notificar(IUnitOfWork uow, long nuReporte, int cdCamion, int cdEmpresa, string cdCliente, string language, byte[] fileData, string fileName)
        {
            ...
            var instancias = uow.EventoRepository.GetInstanciasHabilitadas(EventoDb.ControlCambio, new Dictionary<string, string>
            {
        ...
        public virtual List<long> Preparar()
        {
            ...
            foreach (var empresa in empresas)
            {
                ...
                if (string.IsNullOrEmpty(reportes) || !reportes.Contains($"\"{CReporte.CONTROL_CAMBIO}\""))
                    continue;
